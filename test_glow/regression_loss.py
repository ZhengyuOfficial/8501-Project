...
class RLELoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.flow_model = Glow(in_channel=2, n_flow=16, n_block=1)
    def forward(self, pred, sigma, target, target_weight=None):
        ...
        batch_size, num_keypoints, _ = pred.shape
        if num_keypoints % 2 != 0:
            padding_size = 1
        else:
            padding_size = 0
        #error = error.view(batch_size * num_keypoints, 2, 1, 1)
        #error_reshaped = error.view(batch_size, 2 * num_keypoints, 1, 1).float()   
        padded_error = F.pad(error, (0, 0, 0, padding_size))
        error_reshaped = padded_error.view(batch_size, -1, 1, 2).float()
        
        log_p_sum, logdet, z = self.flow_model(error_reshaped)
        log_phi = log_p_sum - logdet
        log_phi = log_phi.reshape(target.shape[0], target.shape[1], 1)
        #log_phi = log_phi.view(batch_size, num_keypoints, -1)
